...
#define Bootloader_SECURITY_KEY_LENGTH 32
static uint8 Bootloader_KeyNonce[Bootloader_SECURITY_KEY_LENGTH];
static uint8 Bootloader_pKeys[Bootloader_SECURITY_KEY_LENGTH] = {0x41, 0x53, 0x52, 0x36, 0x35, 0x30, 0x31, 0x2D, 0x32, 0x30, 0x31, 0x38, 0x31, 0x30, 0x32, 0x33,
                                                                    0x41, 0x53, 0x52, 0x36, 0x35, 0x30, 0x31, 0x2D, 0x32, 0x30, 0x31, 0x38, 0x31, 0x30, 0x32, 0x33};
static uint8 Bootloader_pKeys_size = 0;
...
#endif /*(CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)*/    
{
    ...
    do
    {
                ...
                if((Bootloader_1_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize >= 3u))
                {
                    ...
                    if(dataOffset == pktSize)
                    {
                        ...
                        *******************************************************************************/
                        ...
                        #if(CY_PSOC4)
                            if(Bootloader_pKeys_size) {
                                encryption(dataBuffer, CYDEV_FLS_ROW_SIZE, Bootloader_pKeys, Bootloader_pKeys_size);
                            }
        ...
        #else    
            if(pktSize && (pktSize<=Bootloader_SECURITY_KEY_LENGTH)) {
                for(int i=0; i<pktSize; i++) {
                    Bootloader_KeyNonce[i] = packetBuffer[Bootloader_1_DATA_ADDR + i];
                    Bootloader_pKeys[i] ^= Bootloader_KeyNonce[i];
                }
                Bootloader_pKeys_size = pktSize;
            }
